...
// Après l'insertion réussie dans offre (dans submit.php)
$stmt = $pdo->prepare("INSERT INTO offre (...) VALUES (...)");
$stmt->execute([...]);

// Vérifier si le proposant existe déjà
$checkProposant = $pdo->prepare("SELECT id FROM proposant WHERE utilisateur_id = ?");
$checkProposant->execute([$_SESSION['utilisateur_id']]);

// Si n'existe pas, le créer
if ($checkProposant->rowCount() == 0) {
    $insertProposant = $pdo->prepare("INSERT INTO proposant (utilisateur_id, description) VALUES (?, 'Nouveau proposant')");
    $insertProposant->execute([$_SESSION['utilisateur_id']]);
}
