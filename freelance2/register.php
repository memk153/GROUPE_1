        ...
        if ($checkStmt->num_rows > 0) {
            ...
            if (!$stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    // Ajout pour enregistrer dans la table proposant si action est 'publier'
                    if ($action === 'publier') {
                        $proposantStmt = $conn->prepare("INSERT INTO proposant (utilisateur_id, description) VALUES (?, ?)");
                        $descriptionParDefaut = "Nouveau proposant - description à compléter";
                        $proposantStmt->bind_param("is", $utilisateur_id, $descriptionParDefaut);
                        
                        if (!$proposantStmt->execute()) {
                            $message = "Erreur lors de la création du profil proposant: " . $conn->error;
                            $proposantStmt->close();
                            $stmt->close();
                            $conn->close();
                            die($message);
